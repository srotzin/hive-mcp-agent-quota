...
const SUBSCRIPTION_TIERS = {
  starter:    { price_usd: 50.0, calls_per_day: 100000, label: 'Starter' },
  pro:        { price_usd: 200.0, calls_per_day: 1000000, label: 'Pro' },
...
async function emitSpectralReceipt({ event_type, did, amount_usd, tool_name, tx_hash, metadata }) {
  ...
  try {
    const body = JSON.stringify({
      issuer_did: 'did:hive:agent-quota',
    ...
    console.warn('[agent-quota] receipt emit failed (non-fatal):', _.message || _);
  }
}
...
app.post('/v1/subscription', async (req, res) => {
  ...
  if (tier !== 'enterprise' && !tx_hash) {
    return res.status(402).json({
      error: 'payment_required',
      x402: {
        type: 'x402', version: '1', kind: 'subscription_agent-quota',
  ...
  await emitSpectralReceipt({
    ...
    tx_hash: tx_hash || null,
    metadata: { tier, service: 'Hive Agent Quota', expires_ms: record.expires_ms },
  });
  ...
  return res.json({
    ...
    partner_attribution: 'Per-agent quota enforcement — Spectral-signed quota attestation. Complements rate-limiting layers.',
    brand: '#C08D23',
